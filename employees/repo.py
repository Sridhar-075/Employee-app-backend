...
async def create(db:AsyncSession, name:str, email:str,password:str,age:int,line1:str,city:str,postalcode:int,country:str,role:str) ->Employee:
    db_employee = Employee(
        name=name, email=email,password_hash =password,age=age,role=role,
        addresses = [
            Address(
            line1 = line1,
            city=city,
...
async def search_by_name(name:str,db:AsyncSession):
    stmt = select(Employee).options(selectinload(Employee.addresses)).where(Employee.deleted_at==None).where(Employee.name==name)
    result = await db.scalars(stmt)
    employees = result.all()
    return employees

async def get_by_id(db:AsyncSession,id:int):
    stmt = select(Employee).options(selectinload(Employee.addresses)).where(Employee.id==id)
    result = await db.scalars(stmt)
...
async def employee_id_add_address(db:AsyncSession,id:int,line1:str,city:str,postalcode:str,country:str):
    stmt = select(Employee).options(selectinload(Employee.addresses)).where(Employee.id==id)
    result = await db.execute(stmt)
    employee = result.scalar_one() 
    employee.addresses.append(Address(line1=line1,city=city,postalcode=postalcode,country=country))
    try:
        await db.commit()
    except IntegrityError:
        await db.rollback()
