...
async def create(db: AsyncSession, body) ->Employee:
    ...
    employee = await repo.create(db,name=body.name.strip(), email=body.email.strip(),password=hashed,age=body.age,line1=body.address.line1,
...
async def add_address(db:AsyncSession,id,body):
    await repo.employee_id_add_address(db,id,body.line1,body.city,body.postalcode,body.country)
    return
